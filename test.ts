...
const installer = new Installer({
  out_path: `${Deno.cwd()}/examples/oak/dist`,
  src_path: `${Deno.cwd()}/examples/oak/app${
...
async function compileExampleApp() {
  const p = Deno.run({
    cmd: [
      ...
      "compile",
      "--output",
      `examples/oak/app${Deno.build.os === "windows" ? ".exe" : ""}`,
      "examples/oak/app.ts",
    ],
  });
  await p.status();
...
Deno.test(`Installer created ${Deno.build.os}`, async () => {
  ...
  const dist = `${Deno.cwd()}/examples/oak/dist/bundle`;
